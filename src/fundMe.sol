...
contract FundMe {
    ...
    mapping(address => bool) public hasRaisedSpamRequest;
    uint256 public spamRequestCount;
    ...
    function raiseSpamRequest(string memory _proof) public {
        require(!hasRaisedSpamRequest[msg.sender], "You have already raised a spam request.");
        require(bytes(_proof).length > 0, "Proof of scam is required.");

        hasRaisedSpamRequest[msg.sender] = true;
        spamRequestCount++;
        spamReports.push(SpamReport(msg.sender, _proof));
    }
